...
function buildRuntime(env: RuntimeEnv): {
  ...
  //
  // `PI_AGENT_BASE_URL` overrides this to target an Anthropic-compatible gateway
  // (e.g. an internal TrueFoundry/OCL proxy at https://…/v1 — the adapter appends
  // "/messages"). Such gateways authenticate with `Authorization: Bearer <token>`
  // rather than the public API's `x-api-key`. AnthropicAdapter forwards the SDK
  // client's `defaultHeaders` verbatim (getSdkClientOptions → createAnthropic
  // `headers`), so in gateway mode we add that header — ANTHROPIC_API_KEY carries
  // the token. apiKey stays set too: harmless for gateways that read x-api-key.
  const gatewayBaseUrl = readEnvString(env, "PI_AGENT_BASE_URL");
  const anthropic = new Anthropic({
    apiKey,
    baseURL: gatewayBaseUrl ?? "https://api.anthropic.com/v1",
    defaultHeaders: gatewayBaseUrl ? { Authorization: `Bearer ${apiKey}` } : undefined,
  });
  return {
